...
import { log, debug, warn } from '../utils/logger';
...
export async function pushSync(
  ...
  debug(`Push found ${files.length} markdown files in "${syncFolder}"`);
  ...
  for (const file of files) {
    ...
    if (file.name === '_index.md') {
      debug(`Push skipping index file "${file.path}"`);
      continue;
    }
    ...
    if (fm) {
      ...
      if (singleBookId && fm.bookstack_book_id !== singleBookId) {
        debug(`Push skipping "${file.path}" — not in selected book (singleBookId=${singleBookId})`);
        continue;
      }
      if (
        !singleBookId &&
        ...
        debug(`Push skipping "${file.path}" — book ${fm.bookstack_book_id} not selected`);
        continue;
      }
      ...
      debug(`Push candidate: "${file.path}", isNew=false`);
      ...
      if (findEntryByPath(mapping, file.path)) {
        debug(`Push skipping "${file.path}" — already in mapping by path`);
        continue;
      }

      const derived = deriveBookAndChapter(file.path, syncFolder);
      if (!derived) {
        debug(`Push skipping "${file.path}" — cannot derive book/chapter from path`);
      ...
      if (singleBookId || settings.selectedBookIds.length > 0) {
        ...
        if (bookId === null) {
          debug(`Push skipping "${file.path}" — book "${derived.bookName}" not found in BookStack`);
          continue;
        }
        ...
        if (!singleBookId && !isBookSelected(bookId, settings.selectedBookIds)) {
          debug(`Push skipping "${file.path}" — book ${bookId} not selected`);
          continue;
        }
      }
      ...
      if (content.trim().length === 0) {
        debug(`Push skipping "${file.path}" — file is empty`);
        continue;
      }

      debug(`Push candidate: "${file.path}", isNew=true, book="${derived.bookName}", chapter=${derived.chapterName ?? 'none'}`);
      candidates.push({
        file,
        frontmatter: null,
        content,
  ...
  if (candidates.length === 0) {
    log('Push — no local changes to push');
    new Notice('BookBridge: No local changes to push');
    return result;
  }

  log(`Push — ${candidates.length} candidate(s) found`);
  ...
  for (const candidate of candidates) {
    ...
    try {
      if (candidate.isNewFile) {
        debug(`Push creating page "${candidate.file.basename}" in book "${candidate.bookName}"${candidate.chapterName ? `, chapter "${candidate.chapterName}"` : ''}`);
        await pushNewPage(
          app,
          client,
          mapping,
        ...
        debug(`Push created page "${candidate.file.basename}" successfully`);
      } else {
        debug(`Push updating page "${candidate.file.basename}" (bookstack_id=${candidate.frontmatter.bookstack_id})`);
        await pushPage(
          app,
          client,
          mapping,
        ...
        debug(`Push updated page "${candidate.file.basename}" successfully`);
      }
    } catch (error) {
...
async function pushNewPage(
  ...
  debug(`Push resolved book "${bookName}" → ID ${bookId}`);
  ...
  if (chapterName) {
    ...
    debug(`Push resolved chapter "${chapterName}" → ID ${chapterId}`);
  }
  ...
  debug(`Push creating page "${file.basename}" with placeholder HTML`);
  ...
  if (assetToUrl.size > 0) {
    ...
    debug(`Push updating page "${file.basename}" with ${assetToUrl.size} uploaded image(s)`);
...
async function uploadLocalImages(
  ...
  debug(`Found ${localPaths.size} local images to upload`);
  ...
  for (const localPath of localPaths) {
    try {
      ...
      if (existing) {
        debug(`Image already exists, skipping upload: ${filename}`);
